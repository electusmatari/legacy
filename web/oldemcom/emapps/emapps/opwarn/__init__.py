...
log = logging.getLogger('opwarn')

def opwarnapp(environ, start_response):
    URLCONF = [
        ('^/window/', view_window),
        ('^/oplist/', view_oplist),
        ('^/', view_info),
...
def view_info(environ):
    ...
    if environ['REQUEST_METHOD'] == 'POST':
        ...
        c.execute("INSERT INTO opwarn_list (title) VALUES (%s)",
                  (optitle,))
        return kgi.redirect_response('http://www.electusmatari.com/opwarn/')
    return kgi.template_response('opwarn/info.html',
                                 user=user)

@require_permission('em')
def view_window(environ):
    return kgi.template_response('opwarn/window.html',
                                 user=environ['emapps.user'])
...
def view_oplist(environ):
    ...
    c.execute("SELECT id, created, title FROM opwarn_list "
              "WHERE created > NOW() - INTERVAL 3 HOUR "
