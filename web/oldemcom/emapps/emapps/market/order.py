...
def view_order(environ, type):
    ...
    return kgi.template_response('market/order.html',
                                 user=user,
                                 type=type,
                                 orders=orders)
...
def expire_orders():
    db = kgi.connect('dbforcer')
    c = db.cursor()
    c.execute("SELECT * FROM market_order WHERE expires < NOW()")
    expired = {}
    cancel = []
    for b in kgi.fetchbunches(c):
        if b.owner not in expired:
            expired[b.owner] = []
        if b.type == 'sell':
            expired[b.owner].append((b.amount, b.item, b.comment))
        cancel.append(b.id)
    for owner in expired:
        sql_create_contract(creator=owner,
                            contracttext=(
                "Orders expired:\n\n" +
                ("\n".join([("%ix %s (%s)" % 
                             (amount, item, comment))
                            for (amount, item, comment)
                            in expired[owner]]))),
                            handler=owner,
                            state="new")
    for oid in cancel:
        sql_cancel_order(owner, oid)
